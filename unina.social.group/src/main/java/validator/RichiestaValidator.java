...
public class RichiestaValidator {
	...
	public void validateStato(String stato) {
	    if (stato == null || stato.isEmpty()) {
            throw new IllegalArgumentException("Lo stato della richiesta non pu� essere nullo o vuoto.");
        }
	...
	public void validateDataAccettazione(LocalDate dataAccettazione, LocalDateTime dataOraInvio, String stato) {
		if (dataAccettazione == null) {
			if(stato.equalsIgnoreCase("accettata")) {
				throw new IllegalArgumentException("La data di accettazione della richiesta non pu� essere nulla se la richiesta � accettata.");
			}
		}
		...
		if (dataAccettazione != null) {
			...
			if (dataAccettazione.isBefore(dataOraInvio.toLocalDate())) {
				throw new IllegalArgumentException("La data di accettazione della richiesta non pu� essere precedente alla data di invio della richiesta stessa.");
			}
			
			if (!stato.equalsIgnoreCase("accettata")) {
				throw new IllegalArgumentException("La data di accettazione della richiesta deve essere nulla se la richiesta non � stata accettata.");
			}
		}
	}
	...
	public void validateUtente(Utente utente) {
		if (utente == null) {
			throw new IllegalArgumentException("Il mittente/destinatario della richiesta non pu� essere nullo.");
		}
